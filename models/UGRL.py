...
class UGRL_GCN_test(nn.Module):
    def __init__(self, n_nb, n_in, n_h, dim_x = 2 ,useact =False, liner=False, dropout = 0.2, useA = True):
        super(UGRL_GCN_test, self).__init__()
        # self.gcn = GCN(n_in, n_h, activation)
        self.gcn_0 = GCN(n_in,n_h*dim_x, act=None)
        self.gcn_1 = GCN( n_h*dim_x , n_h, act=None)
        # self.gcn_at = GAT(n_h * dim_x, n_h)
        self.gcn_2 = GCN(n_h, n_h, act=None)

        self.w = nn.Linear(n_h*dim_x, 16)
        self.a = nn.Linear(16*2, 1)
    ...
    def forward(self, seq_a, seq_p, seq_n, adj=None, diff=None, I =None ):
        ...
        # h_a = F.dropout(h_a_0, self.dropout, training=self.training)
        h_a = self.gcn_1(h_a_0,I)
        # h_a = self.gcn_2(self.act(h_a),I)
        ...
        for seq_n_temp in seq_n_list:
            ...
            # h_n = self.gcn_2(self.act(h_n), adj)
            if self.useact:
                h_n = self.act(h_n)
            h_n_list.append(h_n)
