...
def smooth_loss(x, As, is_norm =True):
    x = x.squeeze()
    N, f = x.shape
...
def run_GCN(args, gpu_id=None, exp_name= None,number = 0 , return_model=False, return_time_series=False):
    ...
    if args.dataset_name in ['Photo', 'DBLP', 'Crocodile', 'CoraFull','WikiCS']:
        ...
        useA = False
    model = UGRL_GCN_test(nb_nodes, nb_feature, args.dim,
...
#               #val_embs = embs[train_d.data.val_mask]
            ...
            for _ in range(2):
                ...
                for i in range(nb_classes):
                    acc_small = torch.sum(preds[test_lbls == i] == test_lbls[test_lbls == i]).float() / test_lbls[test_lbls == i].shape[0]
